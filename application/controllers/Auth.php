...
class Auth extends CI_Controller {
		...
		if($user) {
			...
			if($user['is_active'] == 1) {
				...
				if(password_verify($password, $user['password'])) {
	  			...
	  			E-mail ini belum melakukan aktivasi!
				</div>');
				redirect('auth'); 
	...
	public function registration()
	{
		...
		if($this->form_validation->run() == false){
		   $data['title'] = 'Registrasi Member | TagihanDC';
			...
			$this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">
  			Congratulation! your account has been created. Please activate you account
			</div>');
			 redirect('auth'); 
	...
	$this->email->from('tagihandc@gmail.com', 'Tagihan DC');
	...
	public function verify()
	{
		...
		if($user){
			...
			redirect('auth');
					...
					$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">
  			Account activation failed! Wrong token expired.
			</div>');
			redirect('auth'); 
